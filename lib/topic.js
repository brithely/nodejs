...
exports.home = function(request, response){
  db.query(`SELECT * from topic`, function(error, topics){
    ...
    var html = templet.topic(title, list, `<h2>${title}</h2>${description}`, `<a href="/create">create</a>`);
...
exports.page = function(request, response){
  ...
  db.query(`SELECT * from topic`, function(error, topics){
    ...
    var query = db.query(`SELECT * from topic LEFT JOIN author ON topic.author_id = author.id where topic.id=?`,[queryData.id], function(error2, topic){
      ...
      var list = templet.list(topics);
      var html = templet.topic(title, list, `<h2>${sanitizeHTML(title)}</h2>${sanitizeHTML(description)}
